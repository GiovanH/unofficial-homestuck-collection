...
upload-webapp:
    #!/bin/bash
    chmod -R 755 dist
    rsync -ri --exclude "*.json" --exclude "unofficial-homestuck-collection" --delete-after \
        dist/ "homestuck.giovanh.com:~/www-homestuck/"
    # (cd ../webpage/ && make all)
    chmod -R 755 ../webpage/
    rsync -ri --delete-after \
        ../webpage/build/ "homestuck.giovanh.com:~/www-homestuck-static/unofficial-homestuck-collection/"
