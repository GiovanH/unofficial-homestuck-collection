<template>
  <div class="loadbox">
    <div style="filter: drop-shadow(3px 3px 2px rgba(0, 0, 0, .7));">
    ...
    <p class="loadstage" v-text="loadText"></p>
    <p v-for="v, i in loadEchos"
      class="loadstage-echo"
      :style="{filter: `opacity(${1 - (i/4)})`}"
      :key="i" v-text="v"
    ></p>
  </div>
</template>
...
// cat * | grep "<path d" | sed -E "s/.+?d=\"([^\"]+?)\".+/\"\1\",/g" | sed -E 's/([A-Z ])-([0-9])/\1_\2/g' | sed -E 's/([A-Z ])([0-9])/\1-\2/g' | sed -E 's/([A-Z ])_([0-9])/\1\2/g'
  ...
  computed: {
    loadEchos() {
      return this.loadTextHistory.slice(0, 3)
    },
    loadText() {
  ...
  watch: {
    loadText(to, from) {
      this.loadTextHistory.unshift(from)
    }
  }
}
</script>
